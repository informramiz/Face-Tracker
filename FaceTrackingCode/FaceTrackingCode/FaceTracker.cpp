...
FaceTracker::FaceTracker( )
	...
	MAX_FRAMES_TO_WAIT_FOR_DETECTION_( 10 ),
	frame_count_( 0 ),
	MAX_FRAMES_TO_WAIT( 30 ), no_frames_waited( 0 )
{
	lbp_cascade_frontalface_file_name_ = GetCurrentWorkingDirectory( ) + lbp_cascade_frontalface_file_name_;
	haar_cascade_profileface_file_name_ = GetCurrentWorkingDirectory( ) + haar_cascade_profileface_file_name_;
	haar_cascade_frontalface_file_name_ = GetCurrentWorkingDirectory( ) + haar_cascade_frontalface_file_name_;
...
bool FaceTracker::LoadCascadeClassifiers( )
{
	...
	{
		std::cerr << "Could not load face cascade file : " << lbp_cascade_frontalface_file_name_ << std::endl;
		return false;
	}
...
void FaceTracker::DetectAndTrack(Mat & frame)
{
	...
	if ( !is_face_detected || frame_count_ >= MAX_FRAMES_TO_WAIT_FOR_DETECTION_ )
	{
		bool detection_status = DetectFaces( frontalface_cascade_, frame, false );
	...
	if ( is_face_detected )
	{
		...
		for( int i = 0 ; i < previous_faces_.size() ; i++)
		{
			CamshiftTracker( frame, previous_faces_[i], output_image );
		}
...
{
	VideoCapture video_capture( video_file_name );
...
bool FaceTracker::DetectFaces( cv::CascadeClassifier face_cascade , cv::Mat frame , bool flip_image )
{
	...
	cv::Size face_size = cv::Size( frame_gray.cols/8 , frame_gray.cols/8 );
	//face_cascade.detectMultiScale( frame_gray , faces );
	face_cascade.detectMultiScale ( frame_gray , faces , 1.1 , 2 , 0 | CV_HAAR_SCALE_IMAGE | CV_HAAR_DO_CANNY_PRUNING , face_size );
...
bool FaceTracker::Log( std::string str )
{
	...
	std::ofstream output_file("E:\\VCam_Error.txt", std::ios::app);
